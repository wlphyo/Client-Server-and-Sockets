...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   ifstream temp1(header.filename,ifstream::binary); //open to read
   if(!temp1){
      header.command = cix_command::NAK;
      send_packet (client_sock, &header, sizeof header);
   }else {
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   buffer[header.nbytes]= '\0'; // add null character to string
   ofstream temp1(header.filename,ofstream::binary); // open to write
   if(!temp1){
      header.command=cix_command::NAK;
   }else{
      header.command=cix_command::ACK;
      temp1.write(buffer,header.nbytes);
   }
   send_packet (client_sock, &header, sizeof header);
   delete[] buffer;
}
void reply_rm (accepted_socket& client_sock, cix_header& header) {
   ...
   if(deleted != 0) header.command = cix_command::NAK;
